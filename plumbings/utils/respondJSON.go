...
func FiberJSONWrapWithStatusCreated(c *fiber.Ctx, data any, count ...int) (err error) {
	if len(count) == 1 {
		return c.Status(http.StatusCreated).JSON(fiber.Map{"error": false, "data": data, "count": count[0]})
	}
